...
from database import get_number_of_violation,del_all_violation,sv,engine,metadata,check_student,update_student,del_student,add_student,get_student,del_all_students,get_one_student,get_violator_list,check_true_violation
...
class APPMAIN(tk.Tk):
    ...
    def update_one_student(self):
        ...
        if check == 'yes':
            ...
            lop = self.entry_update_class.get()
            sdt = self.entry_update_phoneNumber.get()
            if check_PhoneNumber(sdt) == False:
                messagebox.showwarning(
                "WARNING", "YOU HAVE NOT ENTERED OR ENTER WRONG STUDENT'S PHONE NUMBER !!!")
                return False
            update_student(self.id_find,name=name,lop=lop,sdt=sdt)
            messagebox.showinfo("Congratulation !!!","Update successful !!!")
            ...
            for i in range(len(list_id)):
                ...
                if list_result[i] == "No mask" or list_result[i] == "Wrong":
                    ...
                    if check:
                        ...
                        name = str(in4[1])
                        text_read = name + "không đeo khẩu trang"
    ...
    def train_after_del(self):
        ...
        if len(list_data)==0:
            self.label_Result['text'] = "Đã xóa thành công sinh viên! Vui lòng Reload lại danh sách!"
    ...
    def find_student(self):
        ...
        if check_id(self.id_find) == False: 
            messagebox.showwarning(
                "WARNING", "ID REQUIRED IS INTEGER !!! YOU ENTERED IN THE WRONG ID!!!")
            return False
        ...
        if self.student_had == False:
            self.label_Result = ttk.Label(self.find_lbFrame,text="Can't search the student you are looking for !!!",font=('Times New Roman', 13),bg = '#f7e8c0')
    ...
    def create_table_students(self):
        ...
        style.configure('Treeview.Heading', background="#D3D3D3", foreground="black",rowheight = 20)
...
class App(tk.Toplevel):
    ...
    def train(self):
        ...
        if self.name_student == "":
            messagebox.showwarning(
                "WARNING", "YOU HAVE NOT ENTERED STUDENT'S NAME !!!")
            return False
        self.class_student = self.name_class.get()
        if self.class_student == "":
            messagebox.showwarning(
                "WARNING", "YOU HAVE N  OT ENTERED STUDENT'S CLASS !!!")
            return False
...
class APPVIOLATION(tk.Toplevel):
    ...
    def create_table_violation(self):
        ...
        self.find_lbFrame = tk.LabelFrame(self, text = 'List Of Violtors',borderwidth=2,font=('Times New Roman', 14),fg='blue',bg = '#f7e8c0')
